...
export default function( revealElement, options ) {
	...
	function toggleAutoSlide( override ) {
		console.log(`toggleAutoSlide: ${JSON.stringify(override, null, 4)}`) //DEBUG
	...
	function cueAutoSlide() {
		...
		if( currentSlide && config.autoSlide !== false ) {
			...
			if( fragmentAutoSlide ) {
				...
				if(!config.loop && currentSlide.querySelectorAll( '.fragment' ).length === 0 ) {
	...
	function pauseAutoSlide() {
		console.log(`autoSlidePaused: ${JSON.stringify(autoSlidePaused, null, 4)}`) //DEBUG
